...
| Domain | [`EntityType`](../libs/api/src/main/java/cc/opencar/assistant/api/EntityType.kt) (`climate`, `cover`, `switch`, …) | Exposed as `domain` (+ legacy `entity`) on `/api/entities`; **selects card family** via client `DOMAIN_CARDS` map. Field inventories: [domains.md](domains.md). Antora bindings: [composites.md](composites.md). |
...
**HU settings (non-VHAL):** There is no `android` product domain. Nav **Conexão** (`group=connect`) holds `switch.wifi` / `switch.bluetooth` (+ ADB/storage helpers). Brightness is `number.brightness` under **Tela** (`group=display`). HA-style **`media_player.vehicle`** and cabin volumes **`number.vol_media`** (0, writable via keys), **`number.vol_navigation`** (2), **`number.vol_voice`** (6), **`number.vol_call`** (3), **`number.vol_ring`** (7) live under **Som** (`group=sound`). Media state `playing` / `paused` / `idle`; attributes `media_title` / `media_artist` / `media_album` / `app_id` / `volume`; writes accept states, transport `play` / `pause` / `play_pause` / `next` / `previous` / `stop`, or volume `volume_up` / `volume_down` / `volume:N`. Now-playing metadata needs the notification listener (or privileged `MEDIA_CONTENT_CONTROL`). Transport and **media** cabin volume writes use unprivileged key inject; levels are read from `Settings.System` `android.car.VOLUME_GROUP/{N}`. Non-media volume groups are read-only without `CAR_CONTROL_AUDIO_VOLUME`. Sound **`media_volume`** stays on VHAL (`SETTING_FUNC_AUDIO_MEDIA_VOLUME`) — likely startup/limit semantics, not the live cabin bus. `speed_volume` / AVAS remain VHAL under **Sound**. The `platform/android.json` fragment is AAOS transport only — see [domains.md](domains.md). Example: pause when gear = P via `set_control` on `media_player.vehicle` = `paused`.
